...
function ModalDemo({ width, maxHeight, paragraphs = 2, label = 'Abrir modal' }: ModalDemoProps) {
    ...
    return (
        <>
            <button className="border rounded-md px-3 py-1.5 cursor-pointer" onClick={() => setIsOpen(true)}>
                {label}
            </button>
                ...
                <Modal.Body>
                    {Array.from({ length: paragraphs }).map((_, i) => (
                        <p key={i} className="mb-3">
                            Párrafo {i + 1}: contenido de ejemplo para verificar el comportamiento del
                            cuerpo con scroll y el espaciado interno del modal.
                        </p>
                    ))}
                </Modal.Body>
                <Modal.Footer>
                    <button className="border rounded-md px-3 py-1.5 cursor-pointer" onClick={() => setIsOpen(false)}>
                        Cerrar
                    </button>
                </Modal.Footer>
            </Modal>
