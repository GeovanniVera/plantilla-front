...
export const StatusComparison: StoryObj = {
    render: () => {
        const statuses: { name: string; badgeVariant: 'success' | 'warning' | 'info'; dotColor: 'green' | 'yellow' | 'blue'; toast: ReturnType<typeof makeToast> }[] = [
            { name: 'Success', badgeVariant: 'success', dotColor: 'green', toast: makeToast('success', 'Operación completada') },
            { name: 'Warning', badgeVariant: 'warning', dotColor: 'yellow', toast: makeToast('warning', 'Revisa tu configuración') },
            { name: 'Info', badgeVariant: 'info', dotColor: 'blue', toast: makeToast('info', 'Novedades disponibles') },
        ]
        const dangerToast = makeToast('error', 'No se pudo completar')

        return (
            <div className="p-6 flex flex-col gap-6" style={{ background: 'var(--bg)' }}>
                {/* Danger has no Badge variant — shown via Toast only */}
                <div className="flex items-center gap-4">
                    <span className="text-xs w-16">danger</span>
