...
function hexToLinear(hex: string): [number, number, number] {
    ...
    return [toLinear(parseInt(m[1], 16) / 255), toLinear(parseInt(m[2], 16) / 255), toLinear(parseInt(m[3], 16) / 255)]
}
...
function linearToOklab([r, g, b]: [number, number, number]): { L: number; a: number; bAxis: number } {
...
export function oklchToHexInGamut(color: Oklch): string {
    let { C } = color
    let lin = oklabToLinear(color.L, C * Math.cos((color.H * Math.PI) / 180), C * Math.sin((color.H * Math.PI) / 180))
    while (gamutOverflow(lin) > 0.02 && C > 0.005) {
        C *= 0.9
        lin = oklabToLinear(color.L, C * Math.cos((color.H * Math.PI) / 180), C * Math.sin((color.H * Math.PI) / 180))
    }
    return linearToHex(gamut_clip(lin))
}

// Local alias kept tiny; contrast.ts remains the single WCAG authority.
const gamut_clip = (lin: [number, number, number]) => lin.map((c) => Math.max(0, Math.min(1, c))) as [number, number, number]
...
export function deriveSemanticPalette(input: SemanticInput): SemanticPalette {
    ...
    const evaluate = (b: string): FgEval => {
        ...
        return vsWhite >= vsInk
            ? { fg: '#ffffff', score: vsWhite }
            : { fg: INK, score: vsInk }
    }
